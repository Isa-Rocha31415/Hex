...
#include <time.h>
#include <unistd.h>
// Se definen el numero maximo de procesos y del string de entrada, ademas de
// losvares para los pipes
#define MAX_PROC 32
#define BUFLEN 128
#define R 0
#define W 1
// Se defina una estrcutura para los pipes y facilitar los areglos de pipes
typedef struct {
  int fda[2];
  int fdb[2];
} pipe_t;
...
void draw_piece(int cx, int cy, char piece) {
  // eSTA FUNCION Pide la coordenada donde se jugo y la pieza que se jugo .
  // Si la pieza no corresponde a un jugador, entonces se sale, sino, tiene los
  // colores definidos. SI es X, la dibuja de azul si Es O es rojo y dibuja el
  // ehxagono pasandoloe las coordenadas
  if (piece == '+')
    return;
...
void draw_pieces(int x0, int y0, int size, char *board) {
  // Esta funcion permite dibujar las piezas se pasanas las coordenadas de
  // juego,el tamaño del tableoro y el tablero Se recorrew el tablero
  // bidirecionalm,ente y se convierten esas coordenadas a la forma 1D, Si se
  // detecta que en el talbero hay una ficha se dibuja la pieza mediante la
  // obtencion de su centro
  int cx, cy;
...
void board_clear(char *board, int size) {
  // Se limpia el tablero, se le pasa el talbero y su tamaño
  for (int i = 0; i < size * size; i++)
    board[i] = '+';
}
int main(int argc, char **argv) {

  // ESte bloque recopila los datos puesto por el usuario
  if (argc != 5) {
  ...
  if (nsim < 1) {
    printf("Wrong number of simulations\n");
    return 1;
  }
  ...
  // Se toma el valor del oponente
  char oponent = (player == 'X') ? 'O' : 'X';
  // SE crea el arreglo de pipes dado a la cantida de procesos hijos
  ...
  // EL padre es 0 y los hijos son diferente de 0
  int rank = 0;
  for (int i = 0; i < nproc; i++) {
    procls[i] = fork();
    if (procls[i] == 0) {
  ...
  // Se inicialia la semilla aleatoria
  ...
  // --- Padre ---
  if (rank == 0) {
    // Cierrra los pipes de escritrua y de lectura
    for (int i = 0; i < nproc; i++) {
      close(pipels[i].fda[W]);
      close(pipels[i].fdb[R]);
    }
    // Se imprime el tablero inicial con sui banneer
    ...
    while (1) {
      ...
      if (c == 'q')
        break;
      if (c == 1) {
        // Adquiere la posicion dode se dio click y se dibuja ese hexagono
        py = gfx_ypos();
        px = gfx_xpos();
      ...
      if (board_test(board, size) != '+') {
        ...
        board[0] = 0;
        for (int i = 0; i < nproc; i++)
          write(pipels[i].fdb[W], board,
                sizeof(board)); // EScribe el padre a los hijos
        break;
      }
      ...
      // El movimeinto completo de la computador
